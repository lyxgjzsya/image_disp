...
def inference(image_pl):
    ...
    w_conv1 = tf.Variable(tf.truncated_normal([3, 3, 3, 32], stddev=1e-2))
    conv1 = tf.nn.conv2d(image_placeholder, w_conv1, [1, 1, 1, 1], padding='SAME')
    b_conv1 = tf.Variable(tf.constant(0.1, shape=[32]))
    a_conv1 = tf.nn.relu(tf.nn.bias_add(conv1, b_conv1))
    pool1 = tf.nn.max_pool(a_conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

    w_conv2 = tf.Variable(tf.truncated_normal([3, 3, 32, 32], stddev=1e-2))
    conv2 = tf.nn.conv2d(pool1, w_conv2, [1, 1, 1, 1], padding='SAME')
    b_conv2 = tf.Variable(tf.constant(0.1, shape=[32]))
    a_conv2 = tf.nn.relu(tf.nn.bias_add(conv2, b_conv2))
    pool2 = tf.nn.max_pool(a_conv2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

    w_fc1 = tf.Variable(tf.truncated_normal([3 * 4 * 32, 100], stddev=1e-2))
    b_fc1 = tf.Variable(tf.constant(0.1, shape=[100]))
    pool2_tmp = tf.reshape(pool2, [-1, 3 * 4 * 32])
