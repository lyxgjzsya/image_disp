...
def inference_old(image_pl):
    ...
    w_fc1 = tf.Variable(tf.truncated_normal([7 * 7 * 64, 1024], stddev=1e-2))
    b_fc1 = tf.Variable(tf.constant(0.1, shape=[1024]))
    pool2_tmp = tf.reshape(pool2, [-1, 7 * 7 * 64])
    a_fc1 = tf.nn.relu(tf.matmul(pool2_tmp, w_fc1) + b_fc1)

    w_fc2 = tf.Variable(tf.truncated_normal([1024,9], stddev=1/1024))
    b_fc2 = tf.Variable(tf.constant(0.001, shape=[9]))
...
def inference(images):
    ...
    with tf.name_scope('softmax_linear'):
        weights = tf.Variable(
            tf.truncated_normal([256, 9],
                                stddev=1.0 / math.sqrt(float(256))),
            name='weights')
        biases = tf.Variable(tf.zeros([9]),
                             name='biases')
